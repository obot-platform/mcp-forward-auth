...
export class ProxyManager {
  private config: Required<Omit<ForwardAuthConfig, "oauth">> & {
    oauth: ForwardAuthConfig["oauth"];
  };
  ...
  private validateConfig(config: ForwardAuthConfig): void {
    if (!config.oauth) {
  ...
  private getDefaultBinaryPath(): string {
    return path.join(__dirname, "..", "bin", "mcp-oauth-proxy");
  }
  ...
  private async isProxyHealthy(): Promise<boolean> {
    try {
      const response = await fetch(
        `http://localhost:${this.config.proxyPort}/health`,
  ...
  private async ensureBinaryExists(): Promise<void> {
    try {
      ...
      throw new Error(
        `mcp-oauth-proxy binary not found at ${this.config.binaryPath}. ` +
  ...
  private async waitForProxy(): Promise<void> {
    ...
    throw new Error(
      `mcp-oauth-proxy failed to become healthy within ${this.config.timeout}ms`,
    );
  }
