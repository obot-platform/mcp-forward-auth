...
import { ForwardAuthConfig, ForwardedHeaders } from "./types";

export class ForwardAuthMiddleware {
  private proxyManager: ProxyManager;
  private oauthPaths: string[];
  ...
  middleware = async (
    ...
    try {
      ...
      const isOAuthPath =
        this.oauthPaths.includes(req.path) ||
        req.path.startsWith("/.well-known");

      // Forward request to the OAuth proxy (including Authorization header)
      const proxyResponse = await this.proxyManager.forwardRequest(
        req.originalUrl || req.url,
        this.getProxiedURL(req),
        new Headers(req.headers as Record<string, string>),
        req.method,
        isOAuthPath && req.method === "POST" ? req.body : undefined,
      );
      ...
      const forwardedHeaders = this.extractForwardedHeaders(
  ...
  private removeForwardedHeaders(req: Request): void {
  ...
  private extractForwardedHeaders(headers: Headers): ForwardedHeaders {
    return {
      ...
      "X-Forwarded-Access-Token":
        headers.get("x-forwarded-access-token") ?? undefined,
    };
  }

  private isAllowedResponseHeader(headerName: string): boolean {
...
export function createForwardAuthMiddleware(config: ForwardAuthConfig) {
